...
public class MPivotAssignment : MonoBehaviour
{
    [SerializeField] string targetTag = null;

    private MPlankCollisionDetection plankCollisionDetection;
    private MPlankRotation plankRotation;

    private void Start()
    {
        plankCollisionDetection = transform.parent.GetComponentInChildren<MPlankCollisionDetection>();
        // Defines plankRotation as PlankRotation script from Plank
        plankRotation = GetComponentInParent<MPlankRotation>();
    }
